...
public class JdbcPartyDao implements PartyDao {
    ...
    public List<Party> findAll(String username){
        ...
        String sql = "SELECT * FROM groups WHERE user_id = ? OR group_id in (select group_id from group_members where member_id = ?)";
...
//    }
    ...
    public List<Party> showUsersByGroupId(int Id) {
        ...
        public boolean setInviteLinkByGroupId(int groupId) {
            ...
            jdbcTemplate.update(sql, "localhost:3000" + "/party/" + groupId + "/invite", groupId);
            return true;
        }
