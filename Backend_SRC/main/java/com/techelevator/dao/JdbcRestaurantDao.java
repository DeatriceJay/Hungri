...
public class JdbcRestaurantDao implements RestaurantDao {
    ...
    public List<Restaurant> getRestaurantByGroupId(int groupId){
        List<Restaurant> restaurant = new ArrayList<>();
        String sql = "SELECT restaurants.*, restaurant_group.total_votes FROM public.restaurants left outer join " +
                "(select count(*) as total_votes, restaurant_id from group_member_vote where group_id = ? and vote group by restaurant_id ) as restaurant_group on \n" +
                "                restaurants.restaurant_id = restaurant_group.restaurant_id";
